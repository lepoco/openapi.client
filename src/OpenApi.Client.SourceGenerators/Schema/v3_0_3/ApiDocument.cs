...
using OpenApi.Client.SourceGenerators.Serialization.Attributes;

namespace OpenApi.Client.SourceGenerators.Schema.v3_0_3;
...
public sealed class ApiDocument : IApiDocument
{
    ...
    {
        return ApiDocumentVersion.v3_0_3;
    }
    ...
    public IEnumerable<ApiDocumentType> GetTypes()
    {
        foreach (KeyValuePair<string, ISchemaType> schema in Components.Schemas)
        {
            ...
            {
                Dictionary<string, string> properies = new();

                if (typedSchema.Properties?.Count > 0)
                {
                    foreach (KeyValuePair<string, ISchemaType> property in typedSchema.Properties)
                    {
                        if (property.Value is SchemaType typedPropertySchema)
                        {
                            properies[property.Key] = typedPropertySchema.Type;
                        }
                    }
                }
                ...
                {
                    Name = schema.Key,
                    Summary = typedSchema.Description,
